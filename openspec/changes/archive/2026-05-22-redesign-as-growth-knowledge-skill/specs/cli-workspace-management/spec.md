...
### Requirement: Support skill-oriented local workflows
The CLI SHALL expose the skill package workflows as first-class local commands or delegations rather than as a standalone agent loop.

#### Scenario: User requests growth knowledge capture
- **WHEN** the user asks to capture, ingest, review, search, or recall growth knowledge
- **THEN** the CLI can invoke the skill-managed local scripts or skill workflows without requiring the standalone `pga` interactive REPL as the primary entry point

#### Scenario: User asks to install the growth knowledge skill
- **WHEN** the user installs or configures the skill for Codex, Claude Code, or OpenCode
- **THEN** the CLI behaves as a lightweight local knowledge toolchain rather than as a new agent platform
